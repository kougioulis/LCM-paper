...
def _from_cp_to_full(adj_cp: torch.Tensor, node_names: str | None=None) -> pd.DataFrame:
    ...
    sorted_names = sorted([y for x in lagged_node_names_list for y in x], key=lambda f: f.split("_t-")[-1] if "_t-" in f else "0")
...
def timing(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        tac = time()
        print(f'Elapsed time: {np.round(tac-tic, 4):2.4f} sec')
...
def read_to_csv(data_path: Path, column_names: list | None=None) -> pd.DataFrame:
    """
    ...
    true_data = true_data.rename(columns=dict(zip(true_data.columns, column_names[:true_data.shape[1]], strict=False)))
...
def check_non_stationarity(df: pd.DataFrame, verbose: bool=False) -> bool:
    ...
    for col in df.columns:
        ...
        _adf, pvalue, _used_lag, _, _, _ = adfuller(df.loc[:, [col]].values)
...
def to_stationary_with_finite_differences(df: pd.DataFrame, order: int=1) -> pd.DataFrame:
    ...
    """
    if not check_non_stationarity(df):
        warnings.warn("Provided time-series sample is stationary. No finite differencing is applied.", stacklevel=2)
        return df
