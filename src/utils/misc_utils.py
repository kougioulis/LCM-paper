...
def run_evaluation_experiments(models: dict,
    ...
    for model_name, model in zip(models.keys(), models.values()):
                ...
                for idx in tqdm(range(len(data[:2000])), desc=f'Shard {shard_idx}'):
                    ...
                    try:
                        ...
                        if model_name == "PCMCI":
                            ...
                            assert pred.shape == Y_cpd.shape, \
                                f"Shape mismatch: pred={pred.shape}, Y={Y_cpd.shape}"
        ...
        # NOTE: Standard errors are computed across all datasets and seeds, while in the thesis text & code computed across seeds only for each method 
