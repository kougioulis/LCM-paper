...
Reproducibility experiments for the paper *"Large Causal Models for Temporal Causal Discovery* by Kougioulis et al, 2026.
...
<img src="assets/arch.jpg" align="middle" alt="Neural architecture of a large causal model (LCM) for temporal causal discovery." />
...
| ---------------------------- | ------------------------------------------ | -------------- |
| `experimental_results.ipynb` | Main experimental benchmarks               | §7.3           |
| `illustrative_example.ipynb` | Loading a pretrained LCM & performing CD   | Appendix       |
| `ablation_experiments.ipynb` | Ablations & optimal training data mixture  | §7.1, 7.2      |
| `exp_model_training_size.ipynb` | Training Data—Model Size Convergence    | §9             |
...
Checkpoints for reproducing experiments are available on Google Drive. Download each folder and place it at the repository root under the `logs` path:

| Notebook | Folder | Local path | Link |
| --- | --- | --- | --- |
| `exp_model_training_size.ipynb` | `training_size` | `logs/` (e.g. `logs/250K/10K/seed42.ckpt`) | [Download](https://drive.google.com/drive/folders/1xbfhCswoRoMTFPQs-JIli9eumPm3CX8x) |
| `exp_ablations.ipynb` | `ablations_model` | `logs/ablations_model/` | [Download](https://drive.google.com/drive/folders/1nq9bGDCUw0iRLkauFeMpkLplPXRf_Mss?usp=sharing) |
| `exp_ablations.ipynb` | `ablations_sim` | `logs/ablations_sim/` | [Download](https://drive.google.com/drive/folders/1nkR4iGDC82VFPI5EO0SbK8C9HQejU9Xj?usp=sharing) |
